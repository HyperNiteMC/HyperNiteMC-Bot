...
const getChannel = <T extends GuildChannel>(user: User, type: 'text' | 'voice'): T => {
    return BotUtils.getGuild().channels.find(g => g.name.startsWith(user.id) && g.type == type) as T;
};

const delChannel = (user: User): Promise<GuildChannel>[] => {
    return BotUtils.getGuild().channels.filter(g => g.name.startsWith(user.id)).map(g => g.delete("User Delete"));
};

const idleChannel = async (user: GuildMember): Promise<Boolean> => {
    return BotUtils.getGuild().channels.filter(g => g.name.startsWith(user.id)).every(g => {
...
const isUsing = (user: User): boolean => {
    return BotUtils.getGuild().channels.some(g => g.name.startsWith(user.id));
};

const addPlayer = (user: User, joiner: User): boolean => {
    const matches: Collection<Snowflake, GuildChannel> = BotUtils.getGuild().channels.filter(g => g.name.startsWith(user.id));
    if (matches.size == 0) return false;
    matches.forEach(g => {
        g.overwritePermissions(joiner, {
            VIEW_CHANNEL: true
        }).then(() => {
...
const delPlayer = (user: User, joiner: User): boolean => {
    ...
    matches.forEach(g => {
        g.replacePermissionOverwrites({
            overwrites: g.permissionOverwrites.filter(g => g.id !== joiner.id)
        }).then(() => {
...
const launchIdleChecker = (): void => {
    setInterval(async () => {
        await BotUtils.getGuild().members.forEach((async (mem) => await checkIdle(mem).catch(r => console.error((r as Error).message))));
    }, 60 * 1000);
};
...
const containPlayer = (joiner: GuildMember): boolean => {
    ...
    for (let [, mem] of guild.members) {
        if (guild.channels.filter(g => g.name.startsWith(mem.user.id))
...
const deleteAll = () => {
    BotUtils.getGuild().members.forEach(user => delChannel(user.user));
};
